...
bbox :: bbox ( const vec3& v1, const vec3& v2 )
{
	if ( v1.x < v2.x )
	{
		minPoint.x = v1.x;
...
bbox&    bbox :: addVertex ( const vec3& v )
{
	if ( v.x < minPoint.x )
		minPoint.x = v.x;
...
{
	vec3    nearPoint = plane.makeNearPoint ( minPoint, maxPoint );

	if ( plane.classify ( nearPoint ) == IN_FRONT )
		return IN_FRONT;

	vec3    farPoint = plane.makeFarPoint ( minPoint, maxPoint );
