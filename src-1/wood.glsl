...
void main (void)
{
	const vec4	c1 = vec4 ( 0.902, 0.694, 0.49, 1.0 );
	const vec4	c2 = vec4 ( 0.439, 0.204, 0.07, 1.0 );
	const vec4	specColor = vec4 ( 0.7, 0.7, 0.0, 1.0 );
	const float	specPower = 70.0;

	vec4	ns = turbulence ( 0.2*pp );
	float	f  = 0.5*(1.0+sin(12.0*sqrt(pp.x*pp.x+pp.z*pp.z) + scale*ns.x));
	vec4	c  = mix( c2, c1, f*f );
